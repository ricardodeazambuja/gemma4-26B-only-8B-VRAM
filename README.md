...
| `scripts/run-diffusion-shim.sh` | Launches the experimental `llama-diffusion-cli` backend inside a Node-based HTTP server shim on port `8082` for `pi`'s diffusion provider. Connects stdout/stdin pipes using a JSONL protocol, enforcing Flash Attention, KV quantization, and GPU-resident sampling controls for optimal VRAM/performance on 8 GB cards. |
...
| `scripts/stop-server.sh` | Stops standard (8080) and diffusion (8082) servers. Run interactively, it checks active processes and prompts which to stop; also reaps any orphaned `llama-diffusion-cli` child processes to prevent VRAM memory leaks. |
